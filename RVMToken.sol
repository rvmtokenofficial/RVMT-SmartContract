...
contract RVMToken is Context, IERC20, Ownable {
    ...
    function transferPreSigned(
        ...
        bytes32 hashedTx = recoverPreSignedHash(
            address(this),
            transferSig,
            _to,
            _value,
    ...
    function increaseApprovalPreSigned(
        ...
        require(_nonce == nonces[from].add(1));
        require(
            _allowances[from][_spender].add(_addedValue).add(_fee) <=
                _balances[from]
        );
        ...
        nonces[from] = _nonce;
        signatures[_signature] = true;
        _allowances[from][_spender] = _allowances[from][_spender].add(
            _addedValue
        );
        _balances[from] = _balances[from].sub(_fee);
        _balances[msg.sender] = _balances[msg.sender].add(_fee);
